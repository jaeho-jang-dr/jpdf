...
class PageData:
    """페이지 데이터"""
    page_num: int
    original_image: Any  # np.ndarray (lazy)
    cleaned_image: Any   # np.ndarray (lazy)
...
class JPDF:
    ...
    def _ocr_image(self, image) -> List[TextBlock]:
        """이미지에서 텍스트 블록 추출"""
        import cv2
        import requests
    ...
    def _inpaint_image(self, image, blocks: List[TextBlock], padding: int = 10, radius: int = 7):
        """텍스트 영역 제거 및 배경 복원"""
        import cv2
        import numpy as np
    ...
    def process_image(
        ...
        """이미지 파일 처리"""
        print(f"JPDF: {Path(image_path).name}")
